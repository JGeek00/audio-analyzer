...
void PhaseVocoder::FFTShift(double* src) {
    const int halfSize = m_n / 2;
    for (int i = 0; i < halfSize; ++i) {
        const double temp = src[i];
        src[i] = src[i + halfSize];
        src[i + halfSize] = temp;
    }
}

void PhaseVocoder::processTimeDomain(const double* src,
        double* mag, double* theta, double* unwrapped) {
    for (int i = 0; i < m_n; ++i) m_time[i] = src[i];
    FFTShift(m_time);
...
void PhaseVocoder::processFrequencyDomain(const double* reals,
        const double* imags, double* mag, double* theta, double* unwrapped) {
    for (int i = 0; i < m_n / 2 + 1; ++i) {
        m_real[i] = reals[i];
        m_imag[i] = imags[i];
    }
...
void PhaseVocoder::reset() {
    for (int i = 0; i < m_n / 2 + 1; ++i) {
        const double omega = (2 * M_PI * m_hop * i) / m_n;
        m_phase[i] = -omega;
        m_unwrapped[i] = -omega;
    }
}
...
void PhaseVocoder::unwrapPhases(double* theta, double* unwrapped) {
    for (int i = 0; i < m_n / 2 + 1; ++i) {
        const double omega = (2 * M_PI * m_hop * i) / m_n;
        const double expected = m_phase[i] + omega;
        const double error = MathUtilities::princarg(theta[i] - expected);
